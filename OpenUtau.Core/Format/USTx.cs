...
namespace OpenUtau.Core.Format {
    public class Ustx {
        ...
        public static readonly string[] required = { DYN, PITD, CLR, GENC, BREC, TENC, VOIC, SHFC };
        ...
        public static void AddDefaultExpressions(UProject project) {
            ...
            project.RegisterExpression(new UExpressionDescriptor("voicing (curve) (UTAU)", VOIC, 0, 100, 100) { type = UExpressionType.Curve });
            project.RegisterExpression(new UExpressionDescriptor("tone shift (curve) (UTAU)", SHFC, -1200, 1200, 0) { type = UExpressionType.Curve });
            project.RegisterExpression(new UExpressionDescriptor("resampler engine (UTAU)", ENG, false, new string[] { "", WorldlineResampler.name }));
            project.RegisterExpression(new UExpressionDescriptor("velocity (UTAU)", VEL, 0, 200, 100));
            project.RegisterExpression(new UExpressionDescriptor("volume (UTAU)", VOL, 0, 200, 100));
            project.RegisterExpression(new UExpressionDescriptor("attack (UTAU)", ATK, 0, 200, 100));
            project.RegisterExpression(new UExpressionDescriptor("decay (UTAU)", DEC, 0, 100, 0));
            project.RegisterExpression(new UExpressionDescriptor("gender (UTAU)", GEN, -100, 100, 0, "g"));
            project.RegisterExpression(new UExpressionDescriptor("breath (UTAU)", BRE, 0, 100, 0, "B"));
            project.RegisterExpression(new UExpressionDescriptor("lowpass (UTAU)", LPF, 0, 100, 0, "H"));
            project.RegisterExpression(new UExpressionDescriptor("modulation (UTAU)", MOD, 0, 100, 0));
            project.RegisterExpression(new UExpressionDescriptor("alternate (UTAU)", ALT, 0, 16, 0));
            project.RegisterExpression(new UExpressionDescriptor("direct (UTAU)", DIR, false, new string[] { "off", "on" }));
            project.RegisterExpression(new UExpressionDescriptor("tone shift (UTAU)", SHFT, -36, 36, 0));
        }
