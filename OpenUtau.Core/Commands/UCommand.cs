...
namespace OpenUtau.Core {
    ...
    // ==========================================
    // [DELTA SYNTH] CUSTOM PROJECT COMMANDS
    // ==========================================

    // 1. คำสั่งปรับความเร็วแยกเฉพาะแทร็ก (Track-Specific Tempo)
    public class TrackTempoCommand : ProjectCommand {
        public readonly int TrackNo;
        public readonly double? NewTrackBpm; // null = ใช้ความเร็วรวมของโปรเจกต์
        public readonly double? OldTrackBpm;

        public TrackTempoCommand(UProject project, int trackNo, double? bpm) : base(project) {
            TrackNo = trackNo;
            NewTrackBpm = bpm;
            // ดึงค่าเก่ามาเก็บไว้สำหรับระบบ Undo
            OldTrackBpm = project.tracks[trackNo].OverrideBpm;
        }

        public override void Execute() {
            project.tracks[TrackNo].OverrideBpm = NewTrackBpm;
            // ประมวลผลโครงสร้างเวลาใหม่ทั้งหมดเพื่อไม่ให้โน้ตคลาดเคลื่อน
            project.ValidateFull(); 
        }

        public override void Unexecute() {
            project.tracks[TrackNo].OverrideBpm = OldTrackBpm;
            project.ValidateFull();
        }

        public override string ToString() => $"Track {TrackNo + 1}: Set custom tempo to {NewTrackBpm?.ToString() ?? "Project Default"}";
    }

    // 2. คำสั่งบังคับ Sync ทุกแทร็กให้ตรงกับความเร็วหลักของโปรเจกต์แบบไร้รอยต่อ (Match All)
    public class SyncAllTracksTempoCommand : ProjectCommand {
        private readonly List<(int trackIdx, double? oldBpm)> history = new();
        private readonly double targetBpm;

        public SyncAllTracksTempoCommand(UProject project, double bpm) : base(project) {
            targetBpm = bpm;
            // บันทึกความเร็วเก่าของทุกแทร็กไว้เผื่อผู้ใช้กด Undo
            for (int i = 0; i < project.tracks.Count; i++) {
                history.Add((i, project.tracks[i].OverrideBpm));
            }
        }

        public override void Execute() {
            // 1. ตั้งความเร็วหลักของโปรเจกต์
            project.tempos[0].bpm = targetBpm;
            
            // 2. ล้างค่าความเร็วเฉพาะของทุกแทร็ก (ให้กลับไปเกาะติดความเร็วหลัก)
            foreach (var track in project.tracks) {
                track.OverrideBpm = null;
            }
            
            // 3. จัดระเบียบแกนเวลาใหม่ ป้องกันโน้ตกระโดดหรือคลาดเคลื่อนจากความเร็วดั้งเดิม
            project.ValidateFull();
        }

        public override void Unexecute() {
            // คืนค่าความเร็วเดิมให้แต่ละแทร็ก
            foreach (var item in history) {
                project.tracks[item.trackIdx].OverrideBpm = item.oldBpm;
            }
            project.ValidateFull();
        }

        public override string ToString() => $"Sync all tracks to global tempo: {targetBpm} BPM";
    }
}
