...
namespace OpenUtau.Core.Vogen {
    ...
    public class VogenMandarinPhonemizer : Phonemizer {
        private static InferenceSession g2p;
        private static InferenceSession prosody;
        private static Dictionary<string, string[]> g2pCache = new Dictionary<string, string[]>();
        ...
        void ProcessPart(IList<Note> notes) {
            ...
            for (int i = 0; i < notes.Count; ++i) {
                var lyric = notes[i].lyric.PadRight(8, '\0');
                for (int j = 0; j < 8; j++) {
